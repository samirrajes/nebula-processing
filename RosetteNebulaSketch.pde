...
void setup() {
  ...
  background(0,10,40);    // BLUE-BLACK
  //background(30,0,0);     // RED-BLACK
  //background(0,20,5);     // GREEN-BLACK
  //background(2,22,49);    // NAVY
  //background(20, 0, 30);  // DARK PURPLE
  blendMode(ADD);
  noFill();
  maxRadius = width/3;
  radius = width/2;
  ...
  for (int x = 0; x < nebula.width; x++) {
    for (int y = 0; y < nebula.height; y++) {
      ...
      if (nebulaDensity > 0.5) {
        ...
        //g = map(nebulaDensity, 0.5, 1, 0, 160); // Adding some green to create the pastel pink
        //b = map(nebulaDensity, 0.5, 1, 0, 160); // Adding some blue to create the pastel pink
      
        // BLUE
        //r = map(nebulaDensity, 0.5, 1, 0, 20); 
        //g = map(nebulaDensity, 0.5, 1, 10, 160); 
